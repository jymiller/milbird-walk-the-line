...
| **Behind the scenes** — 26 confirmed photographs on the real block, at measured GPS | https://jymiller.github.io/milbird-walk-the-line/ |
...
`/api/stage2.json` · `/api/photos.json`, published from `gh-pages`.
...
Later, 53 field photographs — **every one carrying a GPS fix in EXIF** — were opened and judged by
eye, one at a time. The verdicts are written down in [`locates/photo_truth.json`](locates/photo_truth.json),
generated by [`locates/photo_truth.py`](locates/photo_truth.py), which carries the note for every
photograph. Before that they existed only in a conversation, which is not a record.

**26 carry genuine locate paint.** 16 are not the street at all — they are the hackathon venue, its
banners and its screens. 5 are people. 6 are clear ground. That venue contamination is why an earlier
widened colour scan reported 45 of 53 "positive" with blue at 49% of frame: it was measuring a
sponsor's backdrop.

The first positive class this project has had, and it immediately contradicted the detector:

| Colour on the ground | Photographs |
|---|---|
| red | 19 |
| green | 10 |
| yellow | 3 |
| white (USA — the 811 request mark) | 2 |
| orange | 1 |

The detector's most common find was **orange, 63 of 124 regions.** On this block orange appears once.
**It was hunting the wrong colour**, which is the single clearest reason recall is zero.

Measured against it:

| | |
|---|---|
| **Recall** | **0 of 26.** Not low — zero. |
| Why | most positives produce no colour region at all; the rest are rejected by rule |
...
- **No false-positive rate.** Only 6 of the 27 paint-free photos are genuine clean pavement; the rest
  are the venue or people. Recall is measurable, precision is not.
- **The published coordinates were wrong for a day.** `locates.json` interpolated all 124 readings
  along a straight east-west line at one fixed latitude, plotting the clip several blocks east of
  where it was filmed. Rebuilt on the block perimeter, and the photographs now carry measured EXIF
  GPS instead. Anything derived says so on the marker.
